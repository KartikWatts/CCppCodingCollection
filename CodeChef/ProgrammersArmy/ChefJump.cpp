...
int main()
{
    ...
    while (t--)
    {
        ...
        int hourInt = int(hour[0] - '0') * 10;
        hourInt += int(hour[1] - '0');
        string minute = s.substr(3, 2);
        int minuteInt = int(minute[0] - '0') * 10;
        minuteInt += int(minute[1] - '0');
        // cout << hourInt << " " << minuteInt;
        hourInt = hourInt % 12;
        minuteInt = minuteInt / 5;
        float hourDegree = hourInt * 30 + float(minuteInt * 2.5);
        float minDegree = minuteInt * 30;

        float ans1 = abs(hourDegree - minDegree);
        float ans2 = 360 - abs(hourDegree - minDegree);

        cout << min(ans1, ans2) << " degree\n";
    }
