...
int getLen(int currNode, int* arr, bool* visited){
    int newNode= arr[currNode];
    if(visited[newNode]==false){
        visited[newNode]=true;
        return 1+ getLen(newNode, arr, visited);
    }
    return 0;
}
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        int currLen= 1+ getLen(i, a, vis);
        // cout<<"For node "<<i+1<<"\t Length = "<<currLen<<"\n";
        if(currLen> maxLen){
            maxLen=currLen;
            maxSrc=i+1;
        }
