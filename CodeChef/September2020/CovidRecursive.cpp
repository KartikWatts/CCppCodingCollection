...
int getInfected(int v[], bool infected[], int total, int athlethe)
{
    for (int i = 0; i < athlethe; i++)
    {
        if (infected[i] == false && v[i] > v[athlethe])
        {
            infected[i] = true;
            getInfected(v, infected, total, i);
        }
    }
    for (int i = athlethe + 1; i < total; i++)
    {
        if (infected[i] == false && v[i] < v[athlethe])
        {
            infected[i] = true;
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            infected[i] = true;
            // cout << "Total infected if " << i << " is infected: ";
            int currInfected = getInfected(v, infected, n, i);
            // cout << currInfected << endl;
            minInfected = min(minInfected, currInfected);
            maxInfected = max(maxInfected, currInfected);
        }
