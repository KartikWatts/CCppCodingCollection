...
int main()
{
    ...
    int zerosFromFirst[n][bit];
    for (int j = 0; j < bit - 1; j++)
    {
        if (b[0][j] == 0)
            zerosFromFirst[0][j] = 1;
        else
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        {
            if (b[i][j] == 0)
                zerosFromFirst[i][j] = 1 + zerosFromFirst[i - 1][j];
            else
                zerosFromFirst[i][j] = zerosFromFirst[i - 1][j];
        }
    }

    int zerosFromLast[n][bit];
    for (int j = 0; j < bit - 1; j++)
    {
        if (b[n - 1 - 0][j] == 0)
            zerosFromLast[0][j] = 1;
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        {
            if (b[n - i - 1][j] == 0)
                zerosFromLast[i][j] = 1 + zerosFromLast[i - 1][j];
            else
    ...
    // cout << "Zeroes from First Calculation\n";

    // for (int i = 0; i < n; i++)
    // {
    //     cout << "Zeroes in " << i + 1 << " bits from first: ";
    //     for (int j = 0; j < bit - 1; j++)
    //     {
    ...
    // cout << "Zeroes from Last Calculation\n";

    // for (int i = 0; i < n; i++)
    // {
    //     cout << "Zeroes in " << i + 1 << " bits from last: ";
    //     for (int j = 0; j < bit - 1; j++)
    //     {
