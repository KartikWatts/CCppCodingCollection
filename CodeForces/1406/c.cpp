...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            maxCompSize = max(maxCompSize, currSize);
        }
        vector<int> maxCompParent;
        for (int i = 0; i < n; i++)
        {
            int currSize = adj[i].size();
            if (maxCompSize == currSize)
            {
                maxCompParent.emplace_back(i);
            }

            // cout << "Nodes connected to node " << i << ":\t";
            // for (auto it : adj[i])
            // {
            //     cout << it << "\t";
            // }
        ...
        if (maxCompParent.size() == 1)
        {
            int parent = maxCompParent.front();
            int child = adj[parent].front();
            cout << parent + 1 << " " << child + 1 << "\n";
            cout << parent + 1 << " " << child + 1 << "\n";
        }
        else
        {
